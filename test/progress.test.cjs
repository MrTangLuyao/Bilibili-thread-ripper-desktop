...
const {ps,execute,setup,cleanup,actualHash,readLog,waitForWorker,waitForFile,assertPhases}=require("./fixture-client.cjs");
...
test("independent native windows update then uninstall offline with hidden ignored stdio",{skip:process.platform!=="win32",timeout:120000},async()=>{
  ...
  try{
    const result=await execute(path.join(f.bridge,"BTR_Desktop.exe"),["update","--client",f.client,"--version",f.manifest.version,"--sha256",f.manifest.sha256],f.env,60000,true);
    ...
    const log=readLog(f,"update-");assertPhases(log,UPDATE_PHASES);assert.match(log,/Worker exit 0/);
    ...
    const removed=await execute(path.join(installed.installPath,"BTR_Desktop.exe"),["uninstall","--client",f.client],f.env,60000,true);
    ...
    const removeLog=readLog(f,"uninstall-");assertPhases(removeLog,REMOVE_PHASES);
...
test("maintenance windows survive when BTR closes the client that started them",{skip:process.platform!=="win32",timeout:150000},async()=>{
  ...
  try{
    ...
    const updateClient=startFromClient([path.join(f.bridge,"BTR_Desktop.exe"),"update","--client",f.client,"--version",f.manifest.version,"--sha256",f.manifest.sha256]);
    const log=await waitForWorker(f,"update-");
    await updateClient;
    assertPhases(log,UPDATE_PHASES);assert.match(log,/Worker exit 0/);
    assert.ok(await waitForFile(f.marker),"Official fixture did not restart");
    ...
    const removeClient=startFromClient([path.join(installed.installPath,"BTR_Desktop.exe"),"uninstall","--client",f.client]);
    const removeLog=await waitForWorker(f,"uninstall-");
    await removeClient;
    assertPhases(removeLog,REMOVE_PHASES);assert.match(removeLog,/Worker exit 0/);
    assert.deepEqual(fs.readFileSync(f.asar),f.original);
    assert.ok(await waitForFile(f.marker),"Uninstall did not restart the official EXE");
    assert.ok(!fs.existsSync(f.pointer),"Uninstall pointer survived");

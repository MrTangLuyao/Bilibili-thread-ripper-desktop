...
当前版本 **0.9.1.1-d4**，支持官方客户端 **1.18.0**。`0.9.1.1` 跟随浏览器主版本，桌面版依次更新 `d1`、`d2`、`d3`、`d4`，主版本变更后重新从 `d1` 开始。

这一版补上卸载的独立进度窗口。确认卸载后，会显示检查备份、关闭客户端、还原、验证和重新打开的过程。失败会留下错误和卸载日志，操作结束后按钮会恢复，不会一直卡在“正在卸载”。账号、缓存和原始备份都保留。
